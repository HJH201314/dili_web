...
function getComputedStyle() {
  console.log(coverRef.value?.offsetWidth)
  const style: CSSProperties = {};
  if (coverRef.value?.offsetWidth) {
    style.maxHeight = `${coverRef.value?.offsetWidth * 9 / 16}px`;
    style.height = `${coverRef.value?.offsetWidth * 9 / 16}px`;
  }
  return style;
}
...
<template>
  <div class="video-card" :class="{'shadowed': props.shadowed}" >
    <section ref="coverRef" class="cover" :style="getComputedStyle()">
      <picture class="cover-image">
        <img :src="props.coverUrl" alt="cover-image" />
      </picture>
    ...
    <section class="title dili-text-ellipsis">
      {{ props.title }}
    </section>
    ...
    <section v-else class="footer">
      ...
      <span class="footer__name" @click="handleUpNameClick">{{ props.upName }}</span>
      ·
      <DateFormat :date="props.createTime" />
    </section>
  </div>
</template>
...
.video-card {
  ...
  .footer {
    ...
    &__name {
      ...
      &:hover {
        color: darken($color-grey-400, 20%);
      }
    }
  }
}
